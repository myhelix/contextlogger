...
#
environment() {
  export SERVICE_NAME=contextlogger
...
cmd_tests() {
  ...
  docker run \
    ...
    ${LOCAL_DOCKER_TAG} bash -c "bash ./entrypoints/test.sh"
}
...
cmd_local() {
  ...
  docker run \
    -it --rm \
    -v `pwd`:/go/src/github.com/calm/${SERVICE_NAME} \
    -e "AWS_DEFAULT_REGION=${AWS_DEFAULT_REGION}" \
    -e "AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID}" \
    -e "AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY}" \
    -e "AWS_SESSION_TOKEN=${AWS_SESSION_TOKEN}" \
    ${LOCAL_DOCKER_TAG} bash -c "bash ./entrypoints/test.sh ${1}"
}
...
help() {
  ...
  Where COMMAND can be:
      ...
      local {watch}       # Run local docker and test. Add watch for test watch mode
...
# If you wish to access a new function you must add a hook here
#
case "$1" in
  build)
    cmd_build ${@:2};;
